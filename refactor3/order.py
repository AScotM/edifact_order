...
class SegmentGenerator:
    ...
    def nad(cls, qualifier: str, party_id: str, name: Optional[str] = None) -> List[str]:
        segments = [f"NAD+{qualifier}+{party_id}::91'"]
        if name:
            segments.append(f"CTA+IC+{cls.escape_edifact(name)}'")
        return segments
...
def generate_edifact_orders(
    ...
    flat_segments.append(SegmentGenerator.unt(len(flat_segments) + 1, validated_data["message_ref"]))
